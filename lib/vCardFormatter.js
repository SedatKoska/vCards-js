...
(function vCardFormatter() {
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
			...
			formattedVCardString += 'VERSIONx:' + vCard.version + nl();
			...
			if (vCard.cellPhone) {
				...
				vCard.cellPhone.forEach(
					function(number){
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;TYPE="voice,cell";VALUE=uri:tel:' + e(number) + nl();
						} else {
			...
			if (vCard.pagerPhone) {
				...
				vCard.pagerPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;TYPE="pager,cell";VALUE=uri:tel:' + e(number) + nl();
						} else {
			...
			if (vCard.workPhone) {
				...
				vCard.workPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;TYPE="voice,work";VALUE=uri:tel:' + e(number) + nl();
			...
			if (vCard.workFax) {
				...
				vCard.workFax.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;TYPE="fax,work"; VALUE=uri:tel:' + e(number) + nl();
			...
			if (vCard.otherPhone) {
				...
				vCard.otherPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;TYPE="voice,other";VALUE=uri:tel:' + e(number) + nl();
